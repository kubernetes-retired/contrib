...
const (
	...
	CacheSize
	// MetricCount is total number metrics.
	MetricCount
)

// MetricToString maps metrics to their string representation.
var MetricToString = map[int]string{
	CacheHits:       "hits",
	CacheMisses:     "misses",
	CacheEvictions:  "evictions",
	CacheInsertions: "insertions",
	CacheSize:       "cachesize",
}

// Metrics exported by dnsmasq via *.bind CHAOS queries.
type Metrics [MetricCount]int64
...
func (mc *metricsClient) GetMetrics() (ret *Metrics, err error) {
	...
	for i := range *ret {
		var value int64
		value, err = mc.getSingleMetric(MetricToString[i] + ".bind.")
		if err != nil {
			return
		}
		ret[i] = value

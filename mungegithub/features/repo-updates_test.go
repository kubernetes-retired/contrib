...
func TestCanonicalFilePath(t *testing.T) {
	...
	tests := []struct {
		...
		{
			testName:     "Slash Rooted Path",
			inputPath:    "/a/b/c",
			expectedPath: "/a/b",
		},
		{
			testName:     "Slash Rooted Path With Trailing Slash",
			inputPath:    "/foo/bar/baz/",
			expectedPath: "/foo/bar/baz",
		},
	}
	for _, test := range tests {
		if test.expectedPath != getCanonicalFilepath(test.inputPath) {
			t.Errorf("Expected the canonical path for %v to be %v.  Found %v instead", test.inputPath, test.expectedPath, getCanonicalFilepath(test.inputPath))
		}
	}
}

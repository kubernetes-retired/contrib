...
func (i Items) LastDate(matcher Matcher, deflt *time.Time) *time.Time {
	matches := i.Filter(matcher)
	if matches.IsEmpty() {
		return deflt
	}
	return matches.GetLast().Date()
}

// FirstDate returns the date of the first matching event, or deflt if no match
func (i Items) FirstDate(matcher Matcher, deflt *time.Time) *time.Time {
	matches := i.Filter(matcher)
	if matches.IsEmpty() {
		return deflt
	}
	return matches.GetFirst().Date()
}

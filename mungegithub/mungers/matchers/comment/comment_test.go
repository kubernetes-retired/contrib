...
func TestCreationBefore(t *testing.T) {
	...
	if CreatedBefore(
		...
		t.Error("CreatedBefore matches later comment")
	}
	if !CreatedBefore(
		time.Date(2000, 1, 1, 12, 0, 0, 0, time.UTC),
	).Match(&github.IssueComment{
		...
		t.Error("CreatedBefore doesn't match earlier comment")
	}
}
...
func TestCreationAfter(t *testing.T) {
	...
	if !CreatedAfter(
		...
		t.Error("CreatedAfter matches later comment")
	}
	if !CreatedAfter(
		...
		t.Error("CreatedAfter doesn't match earlier comment")
	}
}

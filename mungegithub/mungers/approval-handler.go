...
func (h ApprovalHandler) findPeopleToApprove(ownersPath sets.String, prAuthor string) sets.String {
	...
	for ownersFile := range ownersPath {
		for approver := range h.features.Repos.LeafApprovers(filepath.Join(ownersFile, OWNERS)) {
...
func (h *ApprovalHandler) createMessage(obj *github.MungeObject, ownersMap map[string]sets.String) error {
	...
	for _, path := range sliceOfKeys {
		...
		if approverSet.Len() == 0 {
			...
			link := fmt.Sprintf("https://github.com/%s/%s/blob/master/%v", obj.Org(), obj.Project(), ownersPath)

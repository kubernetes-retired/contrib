...
func construct(srcRepo, srcDir, srcURL, srcBranch, dstRepo, dstSubdir string) (string, error) {
	...
	// TODO: this makes construct() specific for client-go. This keeps README.md,
	// examples/ etc. in the client-go, rather than copying them from src.
	if out, err := exec.Command("find", dstSubdir, "-depth", "-maxdepth", "1", "(", "-name", "examples", "-o", "-name", ".github", "-o", "-name", ".git", "-o", "-name", "README.md", "-o", "-path", dstSubdir, ")", "-prune", "-o", "-exec", "rm", "-rf", "{}", "+").CombinedOutput(); err != nil {

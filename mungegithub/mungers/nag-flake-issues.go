...
var (
	pinger = comment.NewPinger(flakeNagNotifName).
...
func removeStaleFlakeNag(obj *mgh.MungeObject, comments []*github.IssueComment) {
	oldNotifs := comment.FilterComments(comments, comment.And([]comment.Matcher{
		comment.MungerNotificationName(flakeNagNotifName),
		comment.CreatedBefore(*comment.LastComment(comments, comment.HumanActor(), &time.Time{})),
	}))
	for _, notif := range oldNotifs {
		obj.DeleteComment(notif)
	}
}
...
func (NagFlakeIssues) Munge(obj *mgh.MungeObject) {
	...
	notif := pinger.SetTimePeriod(findTimePeriod(obj.Issue.Labels)).PingNotification(
		comments,
		mungerutil.GetIssueUsers(obj.Issue).Assignees.Mention().Join(),
		comment.LastComment(comments, comment.HumanActor(), obj.Issue.CreatedAt),
	)
	if notif != nil {

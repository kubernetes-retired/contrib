...
func addMungeFlags(config *mungeConfig, cmd *cobra.Command) {
	...
	cmd.Flags().StringSliceVar(&config.PRMungersList, "pr-mungers", []string{"blunderbuss", "lgtm-after-commit", "needs-rebase", "ok-to-test", "path-label", "ping-ci", "size", "stale-unit-test", "owner-approval", "submit-queue"}, "A list of pull request mungers to run")

...
// hits the GCE MD server if required.
func NewConfigs(zone, projectID, cluster, host string, kubeletPort, ctrlPort uint, resolution time.Duration) (*monitor.SourceConfig, *monitor.SourceConfig, error) {
	if val, err := getZone(zone); err != nil {
		zone = val
		return nil, nil, err
	}

	if val, err := getProjectID(projectID); err != nil {
		projectID = val
		return nil, nil, err
	}

	if val, err := getCluster(cluster); err != nil {
		cluster = val
		return nil, nil, err
	}

	if val, err := getKubeletHost(host); err != nil {
		host = val
		return nil, nil, err
	}

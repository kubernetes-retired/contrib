...
type MetricsSource interface {
	GetTSReq() (*v3.CreateTimeSeriesRequest, error)
	Name() string
}

// Once polls the backend and puts the data to the given service one time.
func Once(src MetricsSource, gcm *v3.Service, name string) {
	req, err := src.GetTSReq()
	if err != nil {
	...
	createCall := gcm.Projects.TimeSeries.Create(name, req)

...
type PodsEvictionRestriction interface {
	// Evict checks if pod can be safely evicted
	Evict(pod *apiv1.Pod) error
	// CanEvict sends eviction instruction to api client.
...
func (e *podsEvictionRestrictionImpl) Evict(podToEvict *apiv1.Pod) error {
	...
	if !present {
		return fmt.Errorf("Pod not suitable for eviction %v", podToEvict.Name)
	}
...
func (f *podsEvictionRestrictionFactoryImpl) NewPodsEvictionRestriction(pods []*apiv1.Pod) PodsEvictionRestriction {
	...
	for _, pod := range pods {
		...
		if creator == nil {
			glog.V(1).Infof("Pod not replicated %v", pod.Name)
			continue
		}
	...
	for creator, replicas := range livePods {
		...
		if actual < f.minReplicas {
			glog.V(1).Infof("Too few replicas for %v %v/%v. Found %v live pods",
		...
		if evictionBudget > 0 {
			...
			glog.V(1).Infof("Cannot evict pods from %v %v/%v. Eviction budget limit : found %v live pods, configured replicas %v",
				creator.Kind, creator.Namespace, creator.Name, actual, configured)
		}
	}

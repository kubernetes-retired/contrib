...
type ContainerRecommendation struct {
	// Name of the container
	Name string
	// Recommended Cpu quantity
	Cpu resource.Quantity
	// Recommened Memory quantity
...
func (rec *recommenderAPIImpl) GetRecommendation(spec *apiv1.PodSpec) (*Recommendation, error) {
	...
	for i, podContainer := range spec.Containers {
		...
		result[i] = ContainerRecommendation{Name: podContainer.Name, Cpu: cpu, Memory: memory}
	}
...
func fake() *VerticalPodAutoscaler {
	...
	return &VerticalPodAutoscaler{
		Spec: Spec{
			Target:       Target{MatchLabels: map[string]string{"app": "redis"}},

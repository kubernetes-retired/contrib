...
declare -a san_array=(IP:${cert_ip} IP:${service_ip} DNS:kubernetes DNS:kubernetes.default DNS:kubernetes.default.svc DNS:kubernetes.default.svc.${dns_domain})
...
if ! (./easyrsa --batch init-pki
      ...
      ./easyrsa --batch --subject-alt-name="${sans}" build-server-full "${masters[0]}" nopass
...
cp -p "pki/issued/${masters[0]}.crt" "${cert_dir}/server.crt"
cp -p "pki/private/${masters[0]}.key" "${cert_dir}/server.key"

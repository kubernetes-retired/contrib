...
func (config *SubmitQueueConfig) RefreshWhitelist() sets.String {
	...
	if pushUsers, _, err := config.UsersWithAccess(); err != nil {
...
func (config *SubmitQueueConfig) doGenCommitters() error {
	...
	unneededInWhitelist := intersection(pushUsers, existingWhitelist)
	for u := range unneededInWhitelist {
		glog.Infof("%v is a committer. Will remove from whitelist.", u)
	}

	neededInWhitelist := existingWhitelist.Difference(pushUsers)
	if err = writeWhitelist(config.Whitelist, "# remove dups with "+os.Args[0]+" gen-committers", neededInWhitelist); err != nil {

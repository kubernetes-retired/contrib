...
// Firewalls interfaces with the GCE firewall api.
...
type Firewall interface {
	CreateFirewall(name, msgTag, srcRange string, ports []int64, hosts []string) error
	GetFirewall(name string) (*compute.Firewall, error)
	DeleteFirewall(name string) error
	UpdateFirewall(name, msgTag, srcRange string, ports []int64, hosts []string) error
}

...
func (f *fakeFirewallRules) CreateFirewall(name, msgTag, srcRange string, ports []int64, hosts []string) error {
	strPorts := []string{}
	for _, p := range ports {
...
func (f *fakeFirewallRules) UpdateFirewall(name, msgTag, srcRange string, ports []int64, hosts []string) error {
	var exists bool
	strPorts := []string{}
	for _, p := range ports {
		strPorts = append(strPorts, fmt.Sprintf("%v", p))

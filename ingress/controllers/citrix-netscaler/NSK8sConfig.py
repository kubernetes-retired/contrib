...
#
    ...
    def usage():
        ...
        print("")
        print(
            "       NSK8sConfig.py <operation addmac|remmac> <kmasterip> <nsepip> <nsmac> <nsflannelsubnet> <nsflannelsubnetmask[0-32]>")
        print("         where")
...
#
    ...
    def addvxlan(cls, args_):
        ...
        command = 'etcdctl --debug --no-sync --peers http://%s:4001 get /flannel/network/config' % config.kmasterip
        jsoncfg = subprocess.check_output(
            command, stderr=DEVNULL, shell=True).rstrip()
        cfg = json.loads(jsoncfg)
        vni = cfg.get('Backend').get('VNI')
        if not vni:
            vni = '1'
        ...
        kminions = subprocess.check_output(
            command, stderr=DEVNULL, shell=True).split()

        try:

            # Login to Netscaler
            ns_session = NSK8sConfig().NSlogin(
                config.nsip, config.username, config.password)
            ...
            for minion in kminions:
                ...
                vxlan_iptunnel_binding.add(
                    ns_session, new_vxlan_iptunnel_binding)

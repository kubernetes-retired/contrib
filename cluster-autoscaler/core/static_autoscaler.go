...
func (a *StaticAutoscaler) writeStatusConfigMap() {
	...
	if getStatusError == nil {
		...
		configMap.ObjectMeta.Annotations["LastUpdated"] = fmt.Sprintf("%v", statusUpdateTime)
		...
		configMap := &apiv1.ConfigMap{
			ObjectMeta: metav1.ObjectMeta{
				...
				Annotations: map[string]string{
					"LastUpdated": fmt.Sprintf("%v", statusUpdateTime),
				},
			},
	...
	if writeStatusError != nil {
		...
		glog.Infof("Succesfully wrote status configmap with body \"%v\"", statusMsg)
	}
}

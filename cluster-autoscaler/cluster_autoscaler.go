...
var (
	...
	aggressiveEstimation = flag.Bool("aggressive-estimation", false, "More aggressive estimation of how many extra nodes are needed")
		...
		case <-time.After(*scanInterval):
			{
				...
				if len(unschedulablePodsToHelp) == 0 {
					...
					scaledUp, err := ScaleUp(unschedulablePodsToHelp, nodes, cloudProvider, kubeClient, predicateChecker, recorder,
						*maxNodesTotal, *aggressiveEstimation)

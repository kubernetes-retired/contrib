...
* Do not modify the nodes. All nodes within the same node group should have the same capacity, labels and system pods dunning on them
* Specify requests for your pods.
* Use Pod Disruption Budgets to prevent pods from being deleted (if needed).
...
### Are all of the mentioned heristics and timings final?
...
### I have couple nodes with low utilization but they are not scaled down. Why?

CA doesn't remove nodes if they are running system pods, pods without a controller or pods with local storage. 
Also it won't remove a node which has pods that cannot be run elsewhere due to limited resources. Another option
is that the corresponding node group already has the minimum size. CA doesn't scale down if there was a scale up
in the last 10 min.
Please check if this is the case with your cluster.

### I have couple of pending pods but there was no scale up? 

CA doesn't scale up the cluster when expansion of any of the node groups (for which it is configured) will not 
make the pods schedule. One of the reason is that the pod has too big requests (like 100 cpus) or too specific
requests (like node selector) that cannot be fulfilled with the current nodes. The other reason is that all of the 
relevant nodes are at their maximum size.
...
Scale up will periodically try to increase the cluster and move back to the previous size utill the quota arrives or the 

...
func ScaleUp(unschedulablePods []*kube_api.Pod, nodes []*kube_api.Node, cloudProvider cloudprovider.CloudProvider, kubeClient *kube_client.Client,
	...
	aggressiveEstimation bool) (bool, error) {
	...
	for _, nodeGroup := range cloudProvider.NodeGroups() {
		...
		if len(option.pods) > 0 {
			if aggressiveEstimation {
				aggresiveEstimator := estimator.NewBinpackingNodeEstimator(predicateChecker)
				option.nodeCount = aggresiveEstimator.Estimate(option.pods, nodeInfo)
			} else {
				safeEstimator := estimator.NewBasicNodeEstimator()
				for _, pod := range option.pods {
					safeEstimator.Add(pod)
				}
				option.nodeCount, option.debug = safeEstimator.Estimate(nodeInfo.Node())
			}

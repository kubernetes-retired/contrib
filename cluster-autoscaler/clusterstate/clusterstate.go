...
const (
	// Maximum time from the moment the node is registered to the time the node is ready.
...
// NodeDeletionRequiest contains information about the requested node deletion.
...
type ClusterStateRegistry struct {
	...
	nodeDeletionRequests  []*NodeDeletionRequest
	nodes                 []*apiv1.Node
...
func NewClusterStateRegistry(cloudProvider cloudprovider.CloudProvider, config ClusterStateRegistryConfig) *ClusterStateRegistry {
	return &ClusterStateRegistry{
		...
		nodeDeletionRequests:  make([]*NodeDeletionRequest, 0),
...
// RegisterNodeDeletion registers node deletion.
func (csr *ClusterStateRegistry) RegisterNodeDeletion(request *NodeDeletionRequest) {
	csr.Lock()
	defer csr.Unlock()
	csr.nodeDeletionRequests = append(csr.nodeDeletionRequests, request)
}
...
func (csr *ClusterStateRegistry) cleanUp(currentTime time.Time) {
	...
	newNdr := make([]*NodeDeletionRequest, 0)
	for _, ndr := range csr.nodeDeletionRequests {
		if ndr.ExpectedDeleteTime.After(currentTime) {
			newNdr = append(newNdr, ndr)
		}
	}
}
...
func (csr *ClusterStateRegistry) calculateAcceptableRanges(targetSize map[string]int) map[string]AcceptableRange {
	...
	for _, sur := range csr.nodeDeletionRequests {
		val := result[sur.NodeGroupName]
		val.MaxNodes += 1
		result[sur.NodeGroupName] = val
	}
...
// Contains readiness information about a group of nodes.

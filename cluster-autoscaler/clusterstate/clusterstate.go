...
func isNodeNotStarted(node *apiv1.Node) bool {
	for _, condition := range node.Status.Conditions {
		if condition.Type == apiv1.NodeReady &&
			...
			condition.LastTransitionTime.Time.Sub(node.CreationTimestamp.Time) < 5*time.Second {
			return true
		}
	}
	return false
...
// GetUpcomingNodes returns how many new nodes will be added shortly to the node groups.

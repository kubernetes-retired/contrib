...
func calculatePodScore(pods []*kube_api.Pod, nodeTemplate *schedulercache.NodeInfo) []*podInfo {
	...
	for _, pod := range pods {
		...
		score := float64(0)
		if cpuCapacity, ok := nodeTemplate.Node().Status.Capacity[kube_api.ResourceCPU]; ok && cpuCapacity.MilliValue() > 0 {
			score += float64(cpuSum.MilliValue()) / float64(cpuCapacity.MilliValue())
		}
		if memCapacity, ok := nodeTemplate.Node().Status.Capacity[kube_api.ResourceMemory]; ok && memCapacity.Value() > 0 {
			score += float64(memorySum.Value()) / float64(memCapacity.Value())
		}

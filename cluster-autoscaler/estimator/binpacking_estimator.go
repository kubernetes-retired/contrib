...
func (estimator *BinpackingNodeEstimator) Estimate(pods []*kube_api.Pod, nodeTemplate *schedulercache.NodeInfo) int {
	...
	// Calculate score for all pods.
	// Score is defined as cpu_sum/node_capacity + mem_sum/node_capacity. With this pods that
	// have bigger requirements are processed first.
	for _, pod := range pods {
		...
		score := float64(0)
		if cpuCapcaity, ok := nodeTemplate.Node().Status.Capacity[kube_api.ResourceCPU]; ok && cpuCapcaity.MilliValue() > 0 {
			score += float64(cpuSum.MilliValue()) / float64(cpuCapcaity.MilliValue())
		}
		if memCapcaity, ok := nodeTemplate.Node().Status.Capacity[kube_api.ResourceMemory]; ok && memCapcaity.Value() > 0 {
			score += float64(memorySum.Value()) / float64(memCapcaity.Value())
		}
	...
	nodeTemplate.Node().Status.Allocatable = nodeTemplate.Node().Status.Capacity
